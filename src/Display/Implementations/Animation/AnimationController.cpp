...
void AnimationController::tick(const std::uint32_t nowMs) {
    switch (_mode) {
        ...
        case AnimationMode::ScrollBitmap:
            ...
            break;
        case AnimationMode::StaticScene:
            tickStaticScene();
            break;
...
void AnimationController::setScene(const DisplayScene& scene) {
    _mode = AnimationMode::StaticScene;
    _stateMachine.clearSequence();
    _scrollState = ScrollState{};
    _scene = scene;
    _needsRender = true;
...
void AnimationController::tickStaticScene() {
    if (!_needsRender) {
        return;
    }

    _displayBackend.render(_scene);
    _needsRender = false;
}
